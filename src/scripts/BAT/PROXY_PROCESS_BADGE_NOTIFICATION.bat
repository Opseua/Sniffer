...
if not "!actionRun!" == "!actionRun:ON=!" (
	...
	set "eComercial=&"
	!3_BACKGROUND! /NOCONSOLE "reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Internet Settings" /v ProxyEnable /t REG_DWORD /d 0 /F" "reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Internet Settings" /v AutoConfigURL /t REG_SZ /d "http://127.0.0.1:!confPort!/?act=!confSecurityPass!-webfile!eComercial!roo=OPSEUA-NODE-CONNECTION-SERVER!eComercial!mes=!fileProjetos!\Sniffer\src\scripts\BAT\proxy.pac!eComercial!mode=dow" /F"
...
if not "!actionRun!" == "!actionRun:OFF=!" (
	...
	set "pathRes=!local!\z_BODY_RES_!outrosAdd!.txt" & set "pathReq=!local!\z_BODY_REQ_!outrosAdd!.txt" & echo !body! > "!pathReq!" & "!wget!" "--post-file=!pathReq!" "!headers!" --quiet -O "!pathRes!" "http://127.0.0.1:!confPort!/?roo=OPSEUA-CHROME-EXTENSION-USUARIO_0"
	...
	set "pathRes=!local!\z_BODY_RES_!outrosAdd!.txt" & set "pathReq=!local!\z_BODY_REQ_!outrosAdd!.txt" & echo !body! > "!pathReq!" & "!wget!" "--post-file=!pathReq!" "!headers!" --quiet -O "!pathRes!" "http://127.0.0.1:!confPort!/?roo=OPSEUA-CHROME-EXTENSION-USUARIO_3"
	del /F /Q "!pathRes!" & del /F /Q "!pathReq!"
